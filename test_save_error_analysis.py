...
def normalize_text(text, lowercase=False, remove_punctuation=False):
...
def main():
    ...
    # result_groups = {
    #     'Extended': SceneTextDataModule.TEST_EXTENDED,
    # }
    result_groups = {
        'Benchmark (Subset)': SceneTextDataModule.TEST_BENCHMARK_SUB,
    }
    ...
    if 'TUL' in test_set:
        print("\nTUL Breakdown Analysis (Accuracy by Length):")
        tul_loader = datamodule.test_dataloaders(('TUL',))['TUL']
        ...
        bucket_stats = {k: {'correct': 0, 'total': 0} for k in length_buckets}
        
        for imgs, labels in tqdm(iter(tul_loader), desc='Analyzing TUL'):
            ...
            for pred, gt in zip(preds, labels):
                ...
                bucket_key = None
                for key, (min_l, max_l) in length_buckets.items():
                    if min_l <= length <= max_l:
                        bucket_key = key
                        break
                
                if bucket_key:
                    res, _ = evaluate_text(pred, gt, cased=args.cased, punctuation=args.punctuation)
                    bucket_stats[bucket_key]['total'] += 1
                    if res:
                        bucket_stats[bucket_key]['correct'] += 1
        
        print(f"\n{'Length':<10} | {'Accuracy':<8} | {'Count':<6}")
        print("-" * 32)
        for key, stats in bucket_stats.items():
            acc = (stats['correct'] / stats['total'] * 100) if stats['total'] > 0 else 0.0
            print(f"{key:<10} | {acc:>8.2f} | {stats['total']:>6}")
